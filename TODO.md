...
- [x] **#64 DONE. `ui_tap` no longer reports success for a touch that could not have worked.** A review of the whole tool after #62/#63, since both had turned out to be instances of one habit — answering `Tapped successfully` whatever happened.
  ...
  - **#64d Refuse a disabled control**, instead of touching it and reporting success — the symptom was otherwise indistinguishable from a mis-aimed tap. Free; the element already carries `enabled`. *Not verified live: the fixture has no disabled control.*
  ...
  - Covered by TESTING_TOOLS.md **#47**, which checks the refusal and then checks three ordinary taps still work — an over-eager guard would be worse than the bug it prevents.
